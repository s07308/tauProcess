...
Under sufficient follow-up, $\hat{\tau}_a(t)$, where $t = X_{(n_0)} \wedge X_{(n_1)}$ can be treated as $\hat{\tau}_a$. The p-value with respect to the null $H_0: \tau_a = 0$ may be calculated by the following steps:
```{r bootstrap-2}
# install.packages("boot")
library(boot)
